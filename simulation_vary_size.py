...
def sample_Wc(N_spins = 500, lb = 0, ub = 1, step = 0.01, repeat = 1000):
  ...
  # beta_grid = np.concatenate((np.arange(lb,0.95,step),np.arange(0.95,0.99,0.001),np.arange(0.99,1.0001,0.0001)))
...
def main(idx):
  ...
  df = monte_conserve(table_beta_hat, z_sample, Wc_sample,
                   ...
                   rho = 0.1, noise = 0.05, alpha_1 = alpha_1, alpha_2 = alpha_2,
                   bandwidth = None)
